...
*different season* the agent has never seen. Three disjoint sets, three jobs:
  ...
  TRAIN (S1 e1-e7):  Reflexion mines rules + OPRO proposes personas here.
  VAL   (S1 e8-e10): elitist keep-best is decided here (model selection).
  TEST  (S2 e1-e10): the honest headline — generalization to the future.
...
characters have no S2 lines), so callers must handle an empty TEST set.
"""

from __future__ import annotations

TRAIN_EPISODES: tuple[str, ...] = tuple(f"s1e{e}" for e in range(1, 8))
VAL_EPISODES: tuple[str, ...] = tuple(f"s1e{e}" for e in range(8, 11))
TEST_EPISODES: tuple[str, ...] = tuple(f"s2e{e}" for e in range(1, 11))
